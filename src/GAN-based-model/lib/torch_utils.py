...
def cpc_loss(pred, tar, pred_mask, attention_mask):
    ...
    other_sample_neg_inner_products = torch.einsum('nte,nle->nlt', pred, torch.flip(tar, dims=[0]))  # shape: (N, T, L)
    max_logit = max(torch.max(inner_products), torch.max(other_sample_neg_inner_products))
    inner_products = inner_products - max_logit.detach()
    ...
    exp_pos_inner_product = torch.diagonal(exp_inner_products, dim1=1, dim2=2)  # shape: (N, T)
    ...
    sample_cpc = masked_reduce_mean(
        torch.log(1 + mean_neg_exp_inner_product / (exp_pos_inner_product + epsilon)),
        pred_mask,
    )
    cpc = sample_cpc.mean()
    return cpc
